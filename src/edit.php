...
$errors = [];
$input = [
    'partij_code' => $voorraad['partij_code'],
    'locatie' => $voorraad['locatie'],
    'gewicht_kg' => $voorraad['gewicht_kg'],
    'kleur' => $voorraad['kleur'],
    'breedte_cm' => $voorraad['breedte_cm'],
    'lengte_cm' => $voorraad['lengte_cm'],
    'status' => $voorraad['status'],
];
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (empty($errors)) {
        $stmt = $pdo->prepare(
            "UPDATE voorraad SET partij_code = :partij_code, locatie = :locatie, gewicht_kg = :gewicht_kg,
             kleur = :kleur, breedte_cm = :breedte_cm, lengte_cm = :lengte_cm, status = :status
             WHERE id = :id"
        );
        $stmt->execute([
